...
class MP4Remuxer {
    ...
    constructor(config:any) {
        ...
        this._forceFirstIDR = !!(((<any>Browser).chrome &&
            ((<any>Browser).version.major < 50 ||
                ((<any>Browser).version.major === 50 && (<any>Browser).version.build < 2661))));

        // Workaround for IE11/Edge: Fill silent aac frame after keyframe-seeking
        // Make audio beginDts equals with video beginDts, in order to fix seek freeze
        this._fillSilentAfterSeek = ((<any>Browser).msedge || (<any>Browser).msie);
    }
    ...
    */
    ...
    seek(originalDts:any) {
        this._videoSegmentInfoList.clear();
        this._audioSegmentInfoList.clear();
    }

    remux(audioTrack:any, videoTrack:any) {
    ...
    _onTrackMetadataReceived(type:any, metadata:any) {
        let metabox = null;
    ...
    _calculateDtsBase(audioTrack:any, videoTrack:any) {
        if (this._dtsBaseInited) {
            return;
        }
    ...
    _remuxAudio(audioTrack:any) {
        ...
        mdatbox.set((<any>MP4).types.mdat, 4);
