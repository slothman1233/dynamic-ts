...
class MP4 {
    ...
    static init() {
        ...
        for (const name in (<any>MP4).types) {
            if ((<any>MP4).types.hasOwnProperty(name)) {
                (<any>MP4).types[name] = [
                    name.charCodeAt(0),
                    name.charCodeAt(1),
                    name.charCodeAt(2),
        ...
        const constants = (<any>MP4).constants = {};

        (<any>constants).FTYP = new Uint8Array([
        ...
        (<any>constants).STSD_PREFIX = new Uint8Array([
        ...
        (<any>constants).STTS = new Uint8Array([
            0x00, 0x00, 0x00, 0x00, // version(0) + flags
            0x00, 0x00, 0x00, 0x00 // entry_count     0个索引
        ]);

        (<any>constants).STSC = (<any>constants).STCO = (<any>constants).STTS;

        (<any>constants).STSZ = new Uint8Array([
            0x00, 0x00, 0x00, 0x00, // version(0) + flags
            0x00, 0x00, 0x00, 0x00, // sample_size
            0x00, 0x00, 0x00, 0x00 // sample_count
        ]);

        (<any>constants).HDLR_VIDEO = new Uint8Array([
            0x00, 0x00, 0x00, 0x00, // version(0) + flags
        ...
        (<any>constants).HDLR_AUDIO = new Uint8Array([
            0x00, 0x00, 0x00, 0x00, // version(0) + flags
            0x00, 0x00, 0x00, 0x00, // pre_defined
        ...
        (<any>constants).DREF = new Uint8Array([
            0x00, 0x00, 0x00, 0x00, // version(0) + flags
        ...
        // Sound media header
        (<any>constants).SMHD = new Uint8Array([
        ...
        // video media header
        (<any>constants).VMHD = new Uint8Array([
    ...
    // 创建ftyp&moov
    static generateInitSegment(meta:any) {
        if (meta.constructor != Array) {
            meta = [meta];
        }
        const ftyp = (<any>MP4).box((<any>MP4).types.ftyp, (<any>MP4).constants.FTYP);
    ...
    static moov(meta:any) {
        ...
        if (meta.length > 1) { return (<any>MP4).box((<any>MP4).types.moov, mvhd, vtrak, atrak, mvex); } else { return (<any>MP4).box((<any>MP4).types.moov, mvhd, vtrak, mvex); }
    }

    // Movie header box
    static mvhd(timescale:any, duration:any) {
        return (<any>MP4).box((<any>MP4).types.mvhd, new Uint8Array([
    ...
    // Track box
    static trak(meta:any) {
        return (<any>MP4).box((<any>MP4).types.trak, MP4.tkhd(meta), MP4.mdia(meta));
    }

    // Track header box
    static tkhd(meta:any) {
        let trackId = meta.id,
            duration = meta.duration;
        let width = meta.presentWidth,
            height = meta.presentHeight;

        return (<any>MP4).box((<any>MP4).types.tkhd, new Uint8Array([
     ...
     * 				stco
     */
    static mdia(meta:any) {
        return (<any>MP4).box((<any>MP4).types.mdia, MP4.mdhd(meta), MP4.hdlr(meta), MP4.minf(meta));
    }

    // Media header box
    static mdhd(meta:any) {
        const timescale = meta.timescale;
        const duration = meta.duration;
        return (<any>MP4).box((<any>MP4).types.mdhd, new Uint8Array([
    ...
    static hdlr(meta:any) {
        let data = null;
        if (meta.type === 'audio') {
            data = (<any>MP4).constants.HDLR_AUDIO;
        } else {
            data = (<any>MP4).constants.HDLR_VIDEO;
        }
        return (<any>MP4).box((<any>MP4).types.hdlr, data);
    }
...
    一般情况下，“minf”包含一个header box，一个“dinf”和一个“stbl”，其中，header box根据track type（即media handler type）分为“vmhd”、“smhd”、“hmhd”和“nmhd”，“dinf”为data information box，“stbl”为sample table box。下面分别介绍。
    ...
    static minf(meta:any) {
        // header box根据track type（即media handler type）分为“vmhd”、“smhd”、“hmhd”和“nmhd”
        let xmhd = null;
        if (meta.type === 'audio') {
            xmhd = (<any>MP4).box((<any>MP4).types.smhd, (<any>MP4).constants.SMHD);
        } else {
            xmhd = (<any>MP4).box((<any>MP4).types.vmhd, (<any>MP4).constants.VMHD);
        }
        return (<any>MP4).box((<any>MP4).types.minf, xmhd, MP4.dinf(), MP4.stbl(meta));
    }
     ...
     */
    static dinf() {
        const result = (<any>MP4).box((<any>MP4).types.dinf,
        (<any>MP4).box((<any>MP4).types.dref, (<any>MP4).constants.DREF)
        );
        return result;
    }
...
    	*	“stbl”几乎是普通的MP4文件中最复杂的一个box了，首先需要回忆一下sample的概念。
 		* 	sample是媒体数据存储的单位，存储在media的chunk中，chunk和sample的长度均可互不相同，如下图所示。
			 ...
			 * 				stco
		 */
    static stbl(meta:any) {
        const result = (<any>MP4).box((<any>MP4).types.stbl, // type: stbl
            MP4.stsd(meta), // Sample Description Table
            (<any>MP4).box((<any>MP4).types.stts, (<any>MP4).constants.STTS), // Time-To-Sample    因为stts的entry count 为0
            // 所以没有关键帧index 的stss
            // 也没有CTTS box CTTS是记录偏移量
            (<any>MP4).box((<any>MP4).types.stsc, (<any>MP4).constants.STSC), // Sample-To-Chunk
            (<any>MP4).box((<any>MP4).types.stsz, (<any>MP4).constants.STSZ), // Sample size
            (<any>MP4).box((<any>MP4).types.stco, (<any>MP4).constants.STCO) // Chunk offset
        );
        return result;
    }
 ...
 * 会有“VisualSampleEntry”类型信息，对于audio track会有“AudioSampleEntry”类型信息。
		 ...
		 */
    static stsd(meta:any) {
        if (meta.type === 'audio') {
            return (<any>MP4).box((<any>MP4).types.stsd, (<any>MP4).constants.STSD_PREFIX, MP4.mp4a(meta));
        } else {
            return (<any>MP4).box((<any>MP4).types.stsd, (<any>MP4).constants.STSD_PREFIX, MP4.avc1(meta));
        }
    }

    static mp4a(meta:any) {
        ...
        return (<any>MP4).box((<any>MP4).types.mp4a, data, MP4.esds(meta));
    }

    static esds(meta:any) {
        ...
        return (<any>MP4).box((<any>MP4).types.esds, data);
    }
    ...
    static avc1(meta:any) {
        ...
        return (<any>MP4).box((<any>MP4).types.avc1, data, (<any>MP4).box((<any>MP4).types.avcC, avcc));
    }

    // Movie Extends box
    static mvex(meta:any) {
        if (meta.length > 1) { return (<any>MP4).box((<any>MP4).types.mvex, MP4.trex(meta[0]), MP4.trex(meta[1])); } else { return (<any>MP4).box((<any>MP4).types.mvex, MP4.trex(meta[0])); }
    }

    // Track Extends box
    static trex(meta:any) {
        ...
        // }
        return (<any>MP4).box((<any>MP4).types.trex, data);
    }

    // Movie fragment box
    static moof(track:any, baseMediaDecodeTime:any) {
        return (<any>MP4).box((<any>MP4).types.moof, MP4.mfhd(track.sequenceNumber), MP4.traf(track, baseMediaDecodeTime));
    }

    static mfhd(sequenceNumber:any) {
        const data = new Uint8Array([
            0x00, 0x00, 0x00, 0x00,
        ...
        return (<any>MP4).box((<any>MP4).types.mfhd, data);
    }

    // Track fragment box
    static traf(track:any, baseMediaDecodeTime:any) {
        const trackId = track.id;

        // Track fragment header box
        const tfhd = (<any>MP4).box((<any>MP4).types.tfhd, new Uint8Array([
            0x00, 0x00, 0x00, 0x00, // version(0) & flags
            (trackId >>> 24) & 0xFF, // track_ID
            (trackId >>> 16) & 0xFF,
            (trackId >>> 8) & 0xFF,
        ...
        const tfdt = (<any>MP4).box((<any>MP4).types.tfdt, new Uint8Array([
            0x00, 0x00, 0x00, 0x00, // version(0) & flags
        ...
        return (<any>MP4).box((<any>MP4).types.traf, tfhd, tfdt, trun, sdtp);
    }

    // Sample Dependency Type box
    static sdtp(track:any) {
        ...
        return (<any>MP4).box((<any>MP4).types.sdtp, data);
    }

    // Track fragment run box
    static trun(track:any, offset:any) {
        const samples = track.samples || [];
        const sampleCount = samples.length;
        ...
        return (<any>MP4).box((<any>MP4).types.trun, data);
    }

    static mdat(data:any) {
        return (<any>MP4).box((<any>MP4).types.mdat, data);
    }
